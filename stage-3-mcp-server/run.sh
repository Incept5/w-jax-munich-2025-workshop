...
case "$MODE" in
    ...
    agent)
        if [ $# -lt 2 ]; then
            ...
            echo ""
            echo "Usage: $0 agent \"Your task here\" [options]"
            echo ""
            echo "Options:"
            echo "  --model, -m <model>    Use a specific model"
            echo "  --verbose, -v          Show detailed reasoning"
            echo ""
            echo "Examples:"
            echo "  $0 agent \"What's the weather in Paris?\""
            echo "  $0 agent --model qwen2.5:7b \"Tell me about Japan\""
            echo "  $0 agent -m mistral:7b -v \"Your task\""
            exit 1
        fi
    ...
    help|--help|-h)
        ...
        echo "  OLLAMA_MODEL            Default model to use"
        ...
        echo "  $0 agent \"What's the weather in Paris?\""
        echo ""
        echo "  # Run agent with specific model"
        echo "  $0 agent --model qwen2.5:7b \"Tell me about Japan\""
        echo ""
        echo "  # Run agent with model and verbose output"
        echo "  $0 agent -m mistral:7b -v \"Your task\""
        echo ""
